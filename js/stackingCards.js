...
(function () {
    ...
    function stackCardsCallback(entries) {
        ...
        if (entries[0].isIntersecting || window.scrollY < offset) {
...
function openQuestionAccordionsOnWideScreen() {
    if (window.innerWidth > 1023) {
        var questionButtons = document.querySelectorAll('#accordionTechs .accordion-button');
        questionButtons.forEach(function (button) {
            var target = button.getAttribute('data-bs-target');
            var collapseElement = document.querySelector(target);
            if (collapseElement) {
                var accordion = new bootstrap.Collapse(collapseElement, {
                    toggle: false,
                });
                accordion.show();
            }
...
window.addEventListener('load', openQuestionAccordionsOnWideScreen);
window.addEventListener('resize', openQuestionAccordionsOnWideScreen);
